...
func writeFileFromReader(path string, reader io.Reader, context string) (int64, error) {
	outFile, err := os.Create(path)
	if err != nil {
		return 0, fmt.Errorf("opengcs: writeFileFromReader: failed to create %s: %s", path, err)
	}
	defer outFile.Close()
	return copyWithTimeout(outFile, reader, 0, uvmTimeout, context)
}

// copyWithTimeout is a wrapper for io.Copy using a timeout duration
func copyWithTimeout(dst io.Writer, src io.Reader, size int64, timeout time.Duration, context string) (int64, error) {
	logrus.Debugf("opengcs: copywithtimeout: size %d: timeout %d: (%s)", size, timeout, context)
	...
	go func() {
		...
		var copied int64 = 0
		for {
			fmt.Printf("calling io.CopyN\n")
			copied, result.err = io.CopyN(dst, src, 1024)
			fmt.Printf("%s: copy %d\n", context, copied)
			result.bytes += copied
			if copied == 0 { // }|| result.err != nil {
				fmt.Printf("***DONE***\n")
				done <- result
				break
			}
			fmt.Printf("%s: copied so far %d\n", context, result.bytes)
		}
	}()
