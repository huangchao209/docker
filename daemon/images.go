...
func (daemon *Daemon) Map() map[image.ID]*image.Image {
	return daemon.stores[runtime.GOOS].imageStore.Map()
}
...
func (daemon *Daemon) Images(imageFilters filters.Args, all bool, withExtraAttrs bool) ([]*types.ImageSummary, error) {
	...
	// need some work to enumerate the stores for all platforms.
	// ------------------------------------------------------------------------------
	...
	if danglingOnly {
		allImages = daemon.stores[runtime.GOOS].imageStore.Heads()
	} else {
		allImages = daemon.stores[runtime.GOOS].imageStore.Map()
	}
	...
	for id, img := range allImages {
		...
		var size int64
		if layerID != "" {
			l, err := daemon.stores[runtime.GOOS].layerStore.Get(layerID)
			if err != nil {
			...
			size, err = l.Size()
			layer.ReleaseAndLog(daemon.stores[runtime.GOOS].layerStore, l)
			if err != nil {
				return nil, err
			}
		}
		...
		for _, ref := range daemon.stores[runtime.GOOS].referenceStore.References(id.Digest()) {
		...
		if newImage.RepoDigests == nil && newImage.RepoTags == nil {
			if all || len(daemon.stores[runtime.GOOS].imageStore.Children(id)) == 0 {
		...
		if withExtraAttrs {
			...
			if imagesMap == nil {
				...
				allLayers = daemon.stores[runtime.GOOS].layerStore.Map()
